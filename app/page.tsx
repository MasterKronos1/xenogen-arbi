...
const PATHWAY_STAGES = [
  { id: 'groundzero', label: 'GroundZero', current: false, done: true  },
  { id: 'btu',        label: 'BTU',        current: false, done: true  },
  { id: 'skills',     label: 'Skills',     current: true,  done: false },
  { id: 'guuz',       label: 'Guuz',       current: false, done: false },
  { id: 'career',     label: 'Career',     current: false, done: false },
]
...
function GenImageWidget({ url, prompt }: { url: string; prompt?: string }) {
  ...
  // Pollinations can take up to 30s — use a longer timeout approach
  useEffect(() => {
    ...
    img.onerror = () => {
      if (retries < 2) {
        // Retry after 5s — Pollinations may still be generating
        setTimeout(() => setRetries(r => r + 1), 5000)
      } else {
        setStatus('error')
      }
    }
    img.src = url + '&t=' + retries // cache-bust on retry
  }, [url, retries])
  ...
  async function handleShare() {
    if (navigator.share) {
      try { await navigator.share({ title: 'Generated by ARBI', url }); return } catch {}
    }
    ...
    <div className="gen-widget">
      {status === 'error' ? (
        <div className="gen-widget-error">
          ...
          <span style={{cursor:'pointer',color:'var(--btn)',textDecoration:'underline'}}
        ...
        <div className="gen-widget-processing">
          ...
          <div className="gen-widget-label">
            {retries > 0 ? `Retrying... (${retries}/2)` : 'Generating image...'}
          </div>
          {prompt && (
            <div className="gen-widget-prompt">
...
${userMsg}`
    ...
    try {
      ...
      console.log('[sendWithFile] type:', payload.type, 'file:', payload.filename)
