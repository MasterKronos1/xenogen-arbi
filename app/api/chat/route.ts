...
export async function POST(req: Request) {
  ...
  // ── LOAD CONTEXT via adapter (no direct Supabase) ─────────────
  ...
  try {
    ...
    // Ecosystem context is enhancement — don't block on failure
  }
  ...
  // ── SAVE conversation + user message via adapter ──────────────
  try {
  ...
  const systemPrompt = [basePrompt, ecosystemContext, memoryContext]
    .filter(Boolean)
    .join('\n\n')
      ...
      const readable = new ReadableStream({
        async start(controller) {
          try {
            ...
            // ── POST-STREAM: save response + log observations ──
            if (conversationId) {
              try {
                await adapter.insert('messages', {
                  conv_id: conversationId,
                ...
                if (userId !== 'anonymous') {
                  ...
                  for (const obs of observations) {
                    // Use the automated pipeline from ecosystem.ts
